...
function [ALLEEG, EEG, com] = pop_ShowIndMSDyn(ALLEEG,EEG,UseMean,FitPar, MeanSet)
    ...
    if UseMean == true
        [MSClass,gfp] = AssignMStates(EEG,Maps,FitPar,ALLEEG(MeanSet).msinfo.ClustPar.IgnorePolarity,ALLEEG(MeanSet).chanlocs);
    else
        [MSClass,gfp] = AssignMStates(EEG,Maps,FitPar,EEG.msinfo.ClustPar.IgnorePolarity);
    end
